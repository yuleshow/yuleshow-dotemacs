...
(defun mp3tag ()
  (interactive)
  (shell-command-on-region (point-min) (point-max) "opencc -c s2t.json" (current-buffer) t)
  (beginning-of-buffer)
  (replace-string ".mp3" "")
  (beginning-of-buffer)
  (replace-string ".m4a" "")
...
(defun han-format-simple ()
  ...
  (shell-command-on-region (point-min) (point-max) "opencc -c t2s.json" (current-buffer) t)
  (beginning-of-buffer)
  (newline)
  (beginning-of-buffer)
  (replace-string "\n" "\n\n　　")
...
(defun convert-buffer-to-traditional-chinese ()
  "用 OpenCC 將當前 buffer 的簡體中文轉換為繁體中文（使用 s2t.json）。"
  (interactive)
...
(defun convert-buffer-to-simplified-chinese ()
  "用 OpenCC 將當前 buffer 的繁體中文轉換為簡體中文（使用 t2s.json）。"
  (interactive)
  (let ((orig-point (point)))
    (save-excursion
      (shell-command-on-region (point-min) (point-max)
